...
def main():
    ...
    if training_args.do_predict:
        ...
        print(os.path.exists(data_args.data_cache_dir + "/test"))
        #改了这里！
    ...
    if training_args.do_predict:
        ...
        if trainer.is_world_process_zero():
            if training_args.predict_with_generate:
                ...
                # print(predict_results.predictions[9])
                # tokk = tokenizer.tokenize_amr(predict_results.predictions[9])
                # print(tokk)
                graphs = []
                for prediction in predict_results.predictions:
                    graph, status, (lin, backr) = tokenizer.decode_amr(prediction,restore_name_ops=False)
                    print(graph)
                    # print(graph.triples)
                    graphs.append(graph.triples)
                with open("/home/hutu/PersonaDataset/AMRBART/examples/persona_amr_graph.json",'w') as f:
                    json.dump(graphs,f)
